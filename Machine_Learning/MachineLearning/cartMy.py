...
#######################
# Success Evaluation with Holdout Method (Model Validation)
#######################

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.30, random_state=45)

# Installation of the model for the train set
cart_model = DecisionTreeClassifier(random_state=17).fit(X_train, y_train)

# Train Error
y_pred = cart_model.predict(X_train) # We asked for the independent variables of the train set. We got predicted values
y_prob = cart_model.predict_proba(X_train)[:, 1] # We asked for the independent variables of the train set and got the probes. Possible values were taken for AUC
print(classification_report(y_train, y_pred)) # I calculated it based on the dependent variable of the train set and the predictions we made for the dependent variable of the train set.
roc_auc_score(y_train, y_prob) # We calculated the Auc score
...
# As a result, the prediction score decreased when the model encountered a set that it did not see.
# The model is overfitted. It is a normal expectation that the train set error will be low, the success rate will be high, and the test set success rate will be low.
...
#4. Hyperparameter Optimization with GridSearchCV
#######################
# Hyperparameters of the current model
cart_model.get_params()

# We determine the ranges according to the values around the default values.
# We choose two hyperparameters that can prevent overfit.
...
# The first way to create the final model
cart_final = DecisionTreeClassifier(**cart_best_grid.best_params_, random_state=17).fit(X, y)
cart_final.get_params()

# Second way to create the final model (way to assign the best parameters to the model)
cart_final = cart_model.set_params(**cart_best_grid.best_params_).fit(X, y)

# Let's observe the errors in the Final Model
...
# Classification of the variables we have according to their contributions to the Regression classification process with the relevant method
# Our main purpose is to make predictions with low errors, reduce SS, reduce purity measurements, etc.
...
# The model begins to memorize the data set and loses its generalizability ability as the model branches as complexity increases.
# Therefore, after value number 3, Validation Score starts to decrease.
#
...
# When there is more than one set of hyperparameters
...
# Can be done with sklearn version '0.23.1'.
# pip install scikit-learn==0.23.1

print(scompile(cart_final.predict).to('python/code'))

print(scompile(cart_final.predict).to('sqlalchemy/sqlite'))

print(scompile(cart_final.predict).to('excel'))
